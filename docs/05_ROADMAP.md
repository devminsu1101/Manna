...
- [x] **화면 꺼짐 방지** — `useWakeLock` 훅, `ReaderChrome`에서 호출(D-1301~1307).
      ...
      불가** — 배포본에서만 확인된다(D-704에 이어 두 번째 같은 함정)
- [ ] **이전 말씀으로 돌아가기** — 좌하단 chevron 스티키 버튼. 떠날 때 위치를 기억하는
      sessionStorage 점프 스택(사용자 제안). 기록 지점은 선택 시트 한 곳,
      저장 값은 라우트 파라미터가 아니라 "그때 보이던 장". standalone PWA엔 브라우저
      뒤로가기가 없다는 게 애초 동기
- [ ] **선택 시트가 현재 권/장부터 열리기** — 매번 창세기부터 스크롤해야 한다.
      Radix 뷰포트 `scrollTop`을 rect 차분으로 직접 잡는다(시트 진입 애니메이션이 transform이라
      `scrollIntoView`는 못 쓴다)
