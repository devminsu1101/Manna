...
- [x] CSRF 재활성 (2026-09-25, D-3101) — `csrf.spa()` + `apiFetch`. 로그아웃도 토큰을 싣는다
...
- [x] CSRF 재활성 (D-404 → D-3101, 2026-09-25). 쓰기 요청은 `src/lib/api.ts`의 `apiFetch`로 보낸다
...
- [~] 공동체 생성 화면 — `/communities/new`. **UI 완료(2026-09-01)**, 제출은 막아 뒀다.
      `POST /communities`가 이 앱의 **첫 쓰기 API**다. CSRF는 켜졌다(D-3101) — `apiFetch`로 보내면 된다
...
- [~] "기도했어요" — `prayer_logs` (사람 기준, 하루 하나, 익명 D-902).
      **UI 완료(2026-09-01)** — 버튼은 `disabled`, 쓰기 API 대기(CSRF는 D-3101로 해결).
